...
void loop() {
  ...
  // setMotorSpeed(255, 0); // maju
  // delay(3000);
  // setMotorSpeed(0, 255); // maju
  // delay(3000);
  // setMotorSpeed(-255, 0); // maju
  // delay(3000);
  // setMotorSpeed(0, -255); // maju
...
void moveRobot(int throttle, int steering) {
  ...
  Serial.println("----------------------------------------");
  Serial.println("RAW RC VALUES (microseconds):");
  Serial.printf("CH1 (Throttle): %4d μs\n", rc_ch1_value);
  Serial.printf("CH2 (Steering): %4d μs\n", rc_ch2_value);
  Serial.printf("CH3 (Rotate):   %4d μs\n", rc_ch3_value);
  Serial.printf("CH4 (E-Stop):   %4d μs\n", rc_ch4_value);
...
void setMotorSpeed(int leftSpeed, int rightSpeed) {
  // Motor kiri
  // Untuk maju: motor kiri harus berputar CW
  // Untuk mundur: motor kiri harus berputar CCW
  if (leftSpeed > 0) {
    // Maju - CW
  ...
  // Motor kanan
  // Untuk maju: motor kanan harus berputar CCW
  // Untuk mundur: motor kanan harus berputar CW
  if (rightSpeed > 0) {
    // Maju - CCW
  ...
  Serial.printf("Left Motor Speed:    %4d\n", leftSpeed);
  Serial.printf("Right Motor Speed:   %4d\n", rightSpeed);
...
void rotateLeft(int speed) {
  // Rotate left: motor kiri mundur (CCW), motor kanan maju (CCW)
  ledcWrite(PWM_CHANNEL_KIRI_CW, 0);   // Motor kiri CCW (mundur)
  ledcWrite(PWM_CHANNEL_KIRI_CCW, speed);
  ledcWrite(PWM_CHANNEL_KANAN_CW, 0);  // Motor kanan CCW (maju)
  ledcWrite(PWM_CHANNEL_KANAN_CCW, speed);
}

void rotateRight(int speed) {
  // Rotate right: motor kiri maju (CW), motor kanan mundur (CW)
  ledcWrite(PWM_CHANNEL_KIRI_CW, speed);
  ledcWrite(PWM_CHANNEL_KIRI_CCW, 0);  // Motor kiri CCW (maju)
  ledcWrite(PWM_CHANNEL_KANAN_CW, speed);
  ledcWrite(PWM_CHANNEL_KANAN_CCW, 0); // Motor kanan CCW (mundur)
}
