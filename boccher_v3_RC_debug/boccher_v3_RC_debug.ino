...
// #define MOTOR_KANAN_IN1 32   // Direction pin 1 untuk motor kiri   // 27
// #define MOTOR_KANAN_IN2 33   // Direction pin 2 untuk motor kiri   // 14
// #define MOTOR_KANAN_ENA 25   // PWM Enable pin untuk motor kiri    // 12

// // L298N Driver Motor Kiri
// #define MOTOR_KIRI_IN3 27    // Direction pin 1 untuk motor kanan  // 32
// #define MOTOR_KIRI_IN4 14    // Direction pin 2 untuk motor kanan  // 33
// #define MOTOR_KIRI_ENB 12    // PWM Enable pin untuk motor kanan   // 25
...
// #define PWM_CHANNEL_KIRI       0    // Channel PWM untuk ENA (motor kiri)
// #define PWM_CHANNEL_KANAN      1    // Channel PWM untuk ENB (motor kanan)
...
//   ledcAttachPin(MOTOR_KANAN_ENA, PWM_CHANNEL_KIRI);
//   ledcAttachPin(MOTOR_KIRI_ENB, PWM_CHANNEL_KANAN);
...
//   ledcWrite(PWM_CHANNEL_KIRI, 255);
...
//   digitalWrite(MOTOR_KANAN_IN2, LOW);
//   ledcWrite(PWM_CHANNEL_KANAN, 255);
//   digitalWrite(MOTOR_KIRI_IN3, LOW);
...
void processRCCommands() {
  ...
  // Mapping CH2 = Throttle dengan titik maksimum di 3/5 range
  if (rc_ch2_value > RC_CH2_CENTER + RC_DEADZONE) {
    // Throttle maju
    if (rc_ch2_value >= THROTTLE_MIN_POINT) {
      mappedThrottle = maxSpeed;  // Sudah maksimum di 3/5 range
    } else {
      mappedThrottle = map(rc_ch2_value, RC_CH2_CENTER + RC_DEADZONE, THROTTLE_MIN_POINT, 0, maxSpeed);
    }
  } else if (rc_ch2_value < RC_CH2_CENTER - RC_DEADZONE) {
    // Throttle mundur
    if (rc_ch2_value <= THROTTLE_MAX_POINT) {
      mappedThrottle = -maxSpeed;  // Sudah maksimum di 3/5 range
    } else {
      mappedThrottle = map(rc_ch2_value, THROTTLE_MAX_POINT, RC_CH2_CENTER - RC_DEADZONE, -maxSpeed, 0);
    }
  } else {
    mappedThrottle = 0;  // Dalam deadzone
  }

  // Mapping CH1 = Steering dengan titik maksimum di 3/5 range
  if (rc_ch1_value > RC_CH1_CENTER + RC_DEADZONE) {
    // Steering ke kanan
    if (rc_ch1_value >= STEERING_MIN_POINT) {
      mappedSteering = maxSpeed;  // Sudah maksimum di 3/5 range
    } else {
      mappedSteering = map(rc_ch1_value, RC_CH1_CENTER + RC_DEADZONE, STEERING_MIN_POINT, 0, maxSpeed);
    }
  } else if (rc_ch1_value < RC_CH1_CENTER - RC_DEADZONE) {
    // Steering ke kiri
    if (rc_ch1_value <= STEERING_MAX_POINT) {
      mappedSteering = -maxSpeed;  // Sudah maksimum di 3/5 range
    } else {
      mappedSteering = map(rc_ch1_value, STEERING_MAX_POINT, RC_CH1_CENTER - RC_DEADZONE, -maxSpeed, 0);
    }
  } else {
    mappedSteering = 0;  // Dalam deadzone
  }
...
void setMotorSpeed(int FixRightSpeed, int FixLeftSpeed) {
  ...
  // Constrain ke range maksimum motor (-255 to 255)
  FixRightSpeed = constrain(FixRightSpeed, -maxSpeed, maxSpeed);
  FixLeftSpeed = constrain(FixLeftSpeed, -maxSpeed, maxSpeed);
